...
export default function DashboardPage() {
  ...
  // ── State ──────────────────────────────────────────────────────────────────
  const [user, setUser] = useState<{ username: string; slug: string; randomSeed: number } | null>(null);
  ...
  // ── Data fetch ─────────────────────────────────────────────────────────────
  useEffect(() => {
    if (!sessionId) { router.push("/auth"); return; }
    ...
    const load = async () => {
      try {
        ...
        const [userData, home] = await Promise.all([userRes.json(), homeRes.json()]);
        setUser(userData);
        setHomeData(home);
      } catch (err) {
